...
def main_worker(num_gpus, args):
    ...
    if args.blu:
        if num_gpus == 1 or args.gpu is not None:
            ...
            model.module.load_blu_new('blu152_n3.data')
            #model.module.acc_update()
        #model.quant_blu('blu152_new.data')
        print('loaded blu.')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        save_checkpoint({
            ...
            }, is_best, filename=args.arch+'/checkpoint_blu35%d.pth.tar'%(epoch+1))
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        with open('log.txt', 'a') as f:
            f.write(message+'\n')
        if args.stat_blu:
            model.quant_blu('blu152_n35.data')
    return top1.avg
...
def dump_model():
    ...
    def get_layer_params(layer):
        ...
        for block in layer._modules:
            params+=get_BN_block_params(layer._modules[block])
        return params
    model = resnet.ResNet(resnet.Bottleneck, [3, 8, 36, 3])
    # model.load('resnet18\\mergedresnet18.sd')
    checkpoint = torch.load('resnet152\\checkpoint_quant93.pth.tar')
    model.load_state_dict(moduledict_to_dict(checkpoint['state_dict']))
    #model.load_state_dict(checkpoint['state_dict'])
    ...
    with open('checkpoint152_quant93.data','wb') as f:
